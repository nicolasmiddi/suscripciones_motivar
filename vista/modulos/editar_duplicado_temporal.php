...
{  
  $id= htmlentities($_GET['suscriptor']);
  $suscriptor= htmlentities($_GET['suscriptor']);
  $filtro = $_GET['filtro'];
}


// pasar el ID tmp para editar 
if (isset($_POST['suscriptor']))
{  $id= htmlentities($_POST['suscriptor']);
   $filtro = $_POST['filtro'];
    $suscriptor= htmlentities($_POST['suscriptor']);
...
<div class="container shadow p-3 mb-5 bg-white rounded contendor-mot">
    ...
    <div class="container" id="#form_editar">
              <div class="row">
                <h2>EDITAR TEMPORAL <?php echo $suscriptor;?></h2>
                <input type="hidden" name="suscriptor" value="<?php echo $suscriptor;?>">
        ...
        <div>
          ...
          <thead>
                <?php
                  $dashboard -> full_duplicado_tmp($id, $filtro);
                ?>
