...
foreach ($fila as $key => $value) {
  ...
  echo "</table>";}
	...
	public function TablaSuscriptosDuplicadosController ($datos){
		$inner = '';
		if (isset($datos['correo'])) $inner = $inner . 'sug_correo_1 = tmp_correo_1 or '; 
		if (isset($datos['nombre'])) $inner = $inner . 'sug_nombre = tmp_nombre or'; 
		if (isset($datos['apellido'])) $inner = $inner . 'tmp_apellido = sug_apellido or '; 
		if (isset($datos['celular'])) $inner = $inner . 'tmp_celular = sug_celular or '; 
		if (isset($datos['dni'])) $inner = $inner . 'tmp_dni = tmp_dni or'; 

	$inner = substr($inner, 0, -3);	
		$respuesta = SuscriptoresModel::TablaSuscriptosDulpicadosModel($inner); 
	foreach ($respuesta as $key => $value) {

	
	$value['sug_nombre']= html_entity_decode($value['sug_nombre']);
	$value['sug_apellido']= html_entity_decode($value['sug_apellido']);
	$value['sug_dni']= html_entity_decode($value['sug_apellido']);
	$value['sug_celular']= html_entity_decode($value['sug_apellido']);
		...
		<tr>
			...
			<td>'.$value['sug_dni'].'</td>
			<td>'.$value['sug_celular'].'</td>
			<td class="text-center" style="vertical-align: middle; background:white;" ROWSPAN="2"><form method="post" action="editar_suscriptor_duplicado_excel">
					<input type="hidden" value="'.$value['sug_id'].'" name="edit">
					<input type="hidden" value="'.$value['tmp_id'].'" name="dup_edit">
					<button class="btn" ><i class="text-primary fas fa-pencil-alt"></i></button>
					</form></td>
		</tr>
		<tr class="fila-duplicada">
			<td>TMP</td>
			<td>'.$value['tmp_correo_1'].'</td>
			<td>'.$value['tmp_nombre'].'</td>
			<td>'.$value['tmp_apellido'].'</td>
			<td>'.$value['tmp_dni'].'</td>
			<td>'.$value['tmp_celular'].'</td>
		</tr>
	';
	}
}
	public function TablaSuscriptosDuplicadosTemporalController ($datos){
    if ($datos['filtro']=='correo') $fila = 'tmp_correo_1'; 
    if ($datos['filtro']=='nombre') $fila = 'tmp_nombre';
    if ($datos['filtro']=='apellido') $fila = 'tmp_apellido';
    if ($datos['filtro']=='dni') $fila = 'tmp_dni';
    if ($datos['filtro']=='celular') $fila = 'tmp_celular'; 


		$respuesta = SuscriptoresModel::TablaSuscriptosDulpicadosTemporalModel($fila); 


		foreach ($respuesta as $key => $value) {
	
	$suscriptor= html_entity_decode($value['suscrip']);
		...
		if ($suscriptor!=""){
				...
				<tr>
					...
					<td><input class="sus" type="hidden" value="'.$suscriptor.'"><input class="filtro" type="hidden" value="'.$datos['filtro'].'"> 	
						<button id="btn-resumen" type="button" class="btn btn-outline-primary btn-sm"><i class="fas fa-search-plus"></i></button></td>
					<td> 	
						<a href="index.php?action=editar_duplicado_temporal&suscriptor='.$suscriptor.'&filtro='.$datos['filtro'].'" class="btn btn-outline-success btn-sm"><i class="fas fa-edit"></i></a></td>
					</tr>
			';}
	}
}
	public function resumen_duplicado_tmp ($id, $fila){
		
    if ($fila=='correo') $fila = 'tmp_correo_1'; 
    if ($fila=='dni') $fila = 'tmp_dni';
    if ($fila=='celular') $fila = 'tmp_celular'; 
    if ($fila=='apellido') $fila = 'tmp_apellido'; 
    if ($fila=='nombre') $fila = 'tmp_nombre'; 

   //  if (($fila=='nombre')||($fila=='apellido')) 
	  // $id= htmlentities($id);

		$respuesta = SuscriptoresModel::SuscriptosDulpicadosTemporalxVariableModel($id, $fila); 

		$html = '<table class="table"><tr><th>Fecha Insert</th><th>ID TEMP</th><th>Nombre</th><th>Apellido</th><th>Correo</th><th>Celular</th><th>DNI</th></tr>';
		foreach ($respuesta as $key => $value) {
			$html = $html .'<tr><td>'.$value['tmt_fecha'] .'</td><td>'.' tmp_'.$value['tmp_id'] .'</td><td>'.$value['tmp_nombre'] .'</td><td>'.$value['tmp_apellido'] .'</td><td>'.$value['tmp_correo_1'] .'</td><td>'.$value['tmp_celular'] .'</td><td>'.$value['tmp_dni'] .'</td></tr>';
	...
	public function full_duplicado_tmp ($id, $filtro){

    if ($filtro=='correo') $filtro = 'tmp_correo_1'; 
    if ($filtro=='dni') $filtro = 'tmp_dni';
    if ($filtro=='celular') $filtro = 'tmp_celular'; 
    if ($filtro=='apellido') $filtro = 'tmp_apellido'; 
    if ($filtro=='nombre') $filtro = 'tmp_nombre'; 


		$respuesta = SuscriptoresModel::SuscriptosDulpicadosTemporalxVariableModel($id, $filtro); 
